...
(`python scripts/fetch_nicer_response.py`), then run the calibration and detection
benchmarks against the NICER configs directly:
...
(the `*_nicer.yaml` configs mirror the XMM ones and write to `outputs/calibration_nicer/`
and `outputs/detect_nicer/` via their `out_dir` key, so they never collide with the XMM
records). The two central results reproduce.
