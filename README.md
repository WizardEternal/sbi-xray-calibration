...
deviation 0.113, with SBC rank histograms collapsed on 4 of 5 parameters. It had
hit its training epoch cap. SBC and a coverage test flagged it, split-conformal
recalibration brought the deviation from 0.113 to 0.026, and the
...
the continuum. Nested sampling's evidence does flag it (ΔlogZ ≈ −8), which is why
an evidence-based check stays in the loop (see [Speed vs trust](#speed-vs-trust));
a per-spectrum score that catches gain shifts is an open problem.
...
(vermilion ▲, solid) sags below it (over-confident, deviation 0.113) and
split-conformal (dotted) pulls it back to within 0.026. The shaded grey band on
...
Nested sampling (UltraNest) on the exact same Poisson likelihood is ~9 000–13 000×
slower per spectrum than the amortized NPE but agrees with it to 0.04–0.10 of the
prior width on clean spectra, and its Bayesian evidence flags the B4 gain shift
that every per-spectrum trust score misses (ΔlogZ ≈ −8 while D1/D2/D3 sit at
chance). Full table and the evidence-vs-detector cross-check in
...
- **Priors** follow **Barret & Dupourqué 2024 Table 1** (N_H, Γ, kT exact). The one documented deviation: the log-uniform normalization windows are shifted down by a fixed factor, because the EPIC-pn effective area is ~10× NICER's and the raw ranges would give sub-second exposures (10⁴–10⁸ counts). Same widths, same log-uniform convention, see the config headers and `RESULTS.md`.
...
recovery-quality check (mean Pearson r 0.88, all five posteriors shrinking
...
|--------|---------|---------------|----------------------------------------|----------------------------|
| faint  | 98      | 0.010         | 0.66, 0.03, 0.11, 0.93, 0.98 (pass)    | 0.49 / 0.66 / 0.88         |
| medium | 986     | 0.013         | 0.86, 0.19, 0.32, 0.86, 0.45 (pass)    | 0.48 / 0.65 / 0.89         |
| bright (this flow) | 9982 | **0.113** | **0, 1e−21, 0, 2e−14, 6e−40 (fail 4/5)** | **0.36 / 0.51 / 0.76** |

SBC and the coverage test flagged the flow; split-conformal recalibration is the
effective fix (0.36/0.51/0.76 → 0.46/0.64/0.88; deviation 0.113 → 0.026); and the
...
coverage deviation **0.014–0.033**, SBC uniform). The cleanest control is the
...
the count regime. The full variant table is in `RESULTS.md`:

| variant | epochs (cap) | raw cov dev | SBC ks_p_min |
|---|---|---|---|
| production (orig.) | 150 / 150 (cap hit) | **0.113** | ≈ 0 (4/5 fail) |
| reseed 101 / 202 / 303 | 83 / 151 / 116 | 0.033 / 0.031 / 0.022 | pass (202: 1-param flag) |
| **uncapped** | 162 / 400 (converged) | **0.014** | 0.028 (uniform) |
...
|--------|-------------------|-------------------|-------------------|------------------|----------|----------|----------|----------|
| faint  | 0.78 (D1)         | 0.67 (D2)         | 0.57 (D1)≈chance  | 0.58 (D1)≈chance | 0.80     | 0.81     | 0.71     | 0.53     |
| medium | **0.97 (D1)**     | 0.83 (D2)         | 0.54 (D1)≈chance  | 0.51 (D2)≈chance | 0.92     | 0.94     | 0.77     | 0.54     |
| bright | **0.97 (D1)**     | 0.84 (D2)         | 0.66 (D2)         | 0.52 (D2)≈chance | 0.89     | 0.96     | 0.81     | 0.53     |
...
- **B1 (unmodeled Fe-K line):** the per-spectrum PPC (D1) catches it at ≥1000 counts (AUC 0.97), because the line adds localized counts the posterior-predictive check cannot reproduce. At ~100 counts shot noise buries it (best per-spectrum AUC 0.78; D3's 0.80 is the population statistic). Where the line slips through it biases Γ softer, and the signed bias grows with counts: +0.10 at medium, **+0.20 mean / +0.26 median (mean |ΔΓ| 0.46) at bright** for the strongest line (`outputs/diagnostics/dgamma_silent_failure.png`, `outputs/detect/consequence.md`).
- **B2 (partial covering):** among the per-spectrum detectors, the embedding-OOD (D2) is the one that gets lift (0.67/0.83/0.84 > PPC's 0.59/0.64/0.78), because a covering fraction reshapes the whole soft continuum, a global distortion the embedding sees better than the channel-wise PPC. The population test (D3) separates it best (0.81/0.94/0.96 with counts), though that is supervised two-sample separability and not a per-spectrum trust score. The f≈0.9 near-chance behaviour is a weak leak absorbed into N_H, not an anomalous inversion, and AUC rises monotonically as the leak grows.
...
- **B4 (detector gain shift):** not detectable by any detector at any level. All 36 B4 cells span AUC 0.43–0.58 (mean 0.50), flat in counts and in gain strength. D3's population separability is no exception (0.53–0.54, at its ~0.66 cv-accuracy control floor). A gain shift preserves spectral shape (it slides the energy axis by a fraction of a percent), so the NPE folds it into the continuum parameters. The embedding-OOD detector does not catch it either (D2 on B4 = 0.48–0.52), against the initial expectation that it would. Gain miscalibration stays invisible to all three trust scores while distorting the continuum, and a per-spectrum score that catches it remains an open problem.
...
**Speed vs agreement (clean Model-A spine).** NS is ~9 000–13 000× slower per
...
**NS evidence flags what the per-spectrum trust scores cannot.** The evidence flag
is `mean(logZ_misspec − logZ_clean)` at the same level: a more negative ΔlogZ
means the well-specified Model A fits the misspecified spectrum worse, i.e. the
model is flagged. Read it against the Phase-4 detector AUCs for the same (family,
strength, level) cell:

| family | strength | level | n | NS ΔlogZ (mis − clean) | D1 AUC | D2 AUC | D3 AUC | verdict |
|---|---|---|---|---|---|---|---|---|
| B1 Fe-K line | 3e-4 | bright | 6 | **−886.5** | 0.970 | 0.810 | 0.893 | both NS **and** detectors flag it |
| B1 Fe-K line | 3e-4 | medium | 6 | **−81.8**  | 0.972 | 0.846 | 0.917 | both flag it |
...
A 3% detector gain shift is invisible to all three per-spectrum trust scores
(AUC ≈ 0.45–0.48, at or below chance; it preserves spectral shape and the NPE folds
it into the continuum parameters), but nested sampling's evidence still penalizes
it (ΔlogZ −7.8 at medium, −1.9 weakly at bright). The mechanism is the one
Buchner's X-ray model-checking line is built on: an evidence / goodness-of-fit test
asks whether any parameter setting of the model explains the data, which a
posterior-only novelty score (asking only whether a draw is far from the clean
cloud) cannot. This is Buchner+14's BXA evidence-comparison methodology and the
model-discovery framing of Buchner & Boorman 2023, and it is the case for keeping
an evidence-based check (NS / BXA) in the loop even when a fast amortized SBI
posterior is the workhorse: the cheap per-spectrum trust scores have a blind spot
(energy-scale calibration) that the evidence does not. On the loud B1 line both NS
evidence and D1 agree (ΔlogZ −886, AUC 0.97); they agree where the misspecification
is obvious and split on the silent failure mode.

The per-spectrum dumps and the analysis tables are regenerated by
`scripts/analyze_ns_bench.py` into `outputs/ns_bench/` (not committed); the NS
section of `RESULTS.md` carries the capped-row and rejection-timeout accounting in
full.
