...
An SBI posterior for an X-ray spectrum comes back in milliseconds, where nested
sampling takes minutes. Nested sampling also hands you a calibration guarantee and
a goodness-of-fit, and the flow does not. On one real instrument response this repo
measures where an SBI posterior can be trusted, which misspecifications a
per-spectrum score catches, and what recalibration recovers. It is the first
systematic misspecification-detection benchmark for X-ray spectral SBI. The
detector ideas come from the general SBI literature. Running them as a controlled
benchmark on X-ray spectra is new, and one family of model errors gets past all of
them.

It overlaps deliberately with Barret & Dupourqué's NPE-for-X-ray series
(Papers I–III) and with the general SBI-misspecification literature. The scope is
one instrument and single-round amortized NPE (see [Limitations](#limitations)),
and the numbers are reproducible from seed.

![coverage and detection summary](outputs/summary_figure.png)
...
independent of this flow. An unmodeled 6.4 keV Fe-K line is caught by the
...
Panel (a) shows the miscalibrated flow: empirical coverage (mean over the 5 marginal
parameters; since conformal recalibrates the marginals, this is marginal coverage,
joint coverage is not shown) vs nominal credibility. Faint and medium sit on the diagonal. The bright raw curve
...
shift) sits on the chance diagonal. The panel shows one grid cell per family, while
the [Detection](#detection) table below reports the best cell over the whole
strength grid, so the two differ for B3 and B4 at medium. D3 (dotted, colored by
family) is overlaid as a population-separability statistic (see the D3 footnote in
[Detection](#detection)). Colorblind-safe Okabe–Ito palette, dpi 220.
...
No X-ray-specific misspecification-detection benchmark was found through June
2026. The detector lineage is below.
...
- **Schmitt et al. 2023 / 2024** ([arXiv:2112.08866](https://arxiv.org/abs/2112.08866); extended IJCV [arXiv:2406.03154](https://arxiv.org/abs/2406.03154)), MMD on the embedding space, the methodological ancestor of our D2 embedding-OOD detector. Our D2 uses the posterior-trained, un-regularized CNN embedding, a near-sufficient summary the flow learned for inference, whereas Schmitt's method trains an MMD-regularized, deliberately overcomplete summary network specifically to make misspecification detectable. Their Eq. 12–13 give the reason this matters: a misspecification that preserves the summary distribution is provably invisible to any test in that summary space, which is why a gain shift, folded into the continuum parameters, evades a near-sufficient embedding. D2's weaker showing here reflects the near-sufficient embedding. The MMD-overcomplete approach is the natural next attempt (see Limitations).
...
scripts/
  ...
  analyze_detect.py  analyze_ns_bench.py
  make_summary_figure.py  make_support_figs.py  make_coverage_panel.py
  ...
  run_reseed_pack.py  analyze_reseed.py   reseed/uncapped robustness pack and its ROBUST/FRAGILE/MIXED classification
...
tests/  test_simulate / test_train_npe / test_calibrate / test_detect /
  test_analyze_reseed / test_ns_bench / test_analyze_detect / test_run_calibration
  (73 tests)
...
Each runs on a strength grid, and the weakest grid point reproduces clean Model A as
a control.
...
- **B3**: continuum-family swap, powerlaw → a custom analytic thermal bremsstrahlung (`M(E)=K·E⁻¹·exp(−E/kT)`, with the Gaunt factor dropped, an acceptable approximation for a misspecification template; jaxspec has no apec/bremss, and the custom-component path worked, so the custom component is used. Diskbb would have been the alternative). Grid = kT.
...
importance weights dominated by a handful of samples. Paper III avoids that regime
with sequential rounds (200k–400k likelihood evaluations per spectrum), see
[Prior work](#prior-work). For a flow in this state, conformal recalibration, or a
fall back to NS, restores coverage.
...
coverage deviation 0.014–0.033, though SBC still flags two of the three reseeds). The cleanest control is the
...
For context, Barret & Dupourqué I (arXiv:2401.06061 §3.2–3.3) recover excellently
at 10⁴–10⁵ counts with a deliberately restricted prior and never run a rank-based
SBC test. The rank-based calibration test is what this repo adds.

At bright, TARP's area-to-curve reads ATC ≈ −0.002 (recomputed from
`outputs/calibration/bright/tarp.npz`), which looks fine. The `sbi` implementation
integrates ECP−α over α ≥ 0.5 only, and the bright curve's deviation sits almost
entirely below that cut: it bows above the diagonal out to α ≈ 0.73, with
max|ECP−α| = 0.102 at α ≈ 0.19 and mean ECP−α = +0.080 over α < 0.5. Inside the
window the deviation nearly cancels (+0.006 against −0.007), which is where the
−0.002 comes from. The abs-area over the full range is 0.053 at bright, versus
0.005 / 0.012 at faint / medium, so the curve itself, or an unsigned whole-curve
summary, does catch the miscalibration. The committed bright ECP curve:
`outputs/diagnostics/tarp_bright_curve.png`.

SBC rank histograms: `outputs/calibration/{faint,medium,bright}/sbc_ranks.png`.
Coverage panel: `outputs/diagnostics/coverage_panel.png`. Recovery scatters:
...
detectors). D1 and D2 are per-spectrum unlabeled novelty scores: given one
spectrum, how suspicious is it? D3/marginal-C2ST answers a population question
instead, so it sits in its own column group (see the † footnote).
...
per-spectrum trust score. It trains a classifier on each cell's labeled
clean-vs-misspec embedding populations, and its per-spectrum number is that
classifier's out-of-fold class-1 probability. Its control-cell floor is ~0.66
CV-accuracy on cells with no real misspecification (the weakest B1 line, norm 5e-6,
is a negligible perturbation, and there D3 reports cv-accuracy ≈ 0.66 while its ROC
AUC sits at ≈ 0.33–0.54, at or below chance). Read every D3 number against that
~0.66 cv-accuracy / ~0.5-AUC null, not against 0.5.

An analyst works one spectrum at a time, so D1/D2 are the operative numbers. D3
...
- **B2** (partial covering): among the per-spectrum detectors, the embedding-OOD (D2) is the one that gets lift (0.67/0.83/0.84 > PPC's 0.59/0.64/0.78), because a covering fraction reshapes the whole soft continuum, a global distortion the embedding sees better than the channel-wise PPC. The population test (D3) separates it best (0.81/0.93/0.96 with counts). The f≈0.9 near-chance behaviour is a weak leak absorbed into N_H, not an anomalous inversion, and AUC rises monotonically as the leak grows.
...
k-NN OOD distance (k-NN is primary, because Mahalanobis whitening amplifies the huge
log-uniform brightness axis and washes out the signal). D3 is the simplified marginal
C2ST: a per-cell CV classifier supervised on the cell's labeled clean-vs-misspec
embedding populations, whose per-spectrum number is the classifier's out-of-fold
class-1 probability, read against its control-cell null (see the † footnote). The
per-spectrum conditional C2ST was pathological against the over-confident NPE
posteriors, since a tight replicate cluster is trivially separable from the broad
clean cloud for clean and misspecified spectra alike, so its AUC carried no
signal.
...
spectra at medium/bright). What does the trust NS gives you cost, and does its
Bayesian evidence catch the misspecifications the per-spectrum scores miss?

Speed vs agreement, on the clean Model-A spine: NS is ~8 800–13 000× slower per
...
| family | strength | level | n | ΔlogZ (count-controlled) [95% CI] | D1 AUC | D2 AUC | D3 AUC | reading |
|---|---|---|---|---|---|---|---|---|
...
```powershell
.venv\Scripts\python.exe scripts\run_calibration.py      --config configs\calibration_nicer.yaml
.venv\Scripts\python.exe scripts\run_detect_benchmark.py --config configs\detect_nicer.yaml
```

(the `*_nicer.yaml` configs mirror the XMM ones except training: `train_npe_nicer.yaml`
raises `max_num_epochs` from the XMM production's 150 to 350 and trains on
`device: cuda` instead of `cpu`, and they write to `outputs/calibration_nicer/` and
`outputs/detect_nicer/` via their `out_dir` key, so they never collide with the XMM
records). The committed NICER detection grid covers 117 of the XMM grid's 144 cells:
B2, the partial-covering family, is absent at medium and bright, and at faint only
3 of its 4 strengths (0.5, 0.7, 0.9) are present. The two central results reproduce.
...
1. Single instrument response. Everything is one bundled XMM-Newton EPIC-pn observation. The calibration trend and the detection AUCs are response-specific, and a different effective-area curve / channel grid would shift the numbers.
2. Single-round amortized NPE, no sequential rounds (Paper III's regime). This is why the IS-refinement is ESS-starved at high counts. The ESS behaviour reflects the single-round setup, so it says nothing about the IS method itself.
3. B3 brems is Gaunt-free analytic. The B3 continuum is `K·E⁻¹·exp(−E/kT)` with the slowly-varying Gaunt factor dropped. It is a wrong-continuum-family template. It is not a calibrated plasma model, and it is not meant to be. A real apec/bremss comparison would need PyXspec on the VM.
4. Gain shifts are undetectable by these three detectors, and a per-spectrum score that catches them is an open problem. B4 is at chance for D1/D2/D3 at all levels. D2 uses the flow's near-sufficient posterior-trained embedding, which a gain shift slides past for the reason given under Schmitt et al. in [Prior work](#prior-work). The natural next attempt is their MMD-regularized overcomplete summary, or a detector that models the energy-scale calibration directly.
5. Counts are controlled via the prior + exposure, with a heavy-tailed log-uniform norm, and the median count is the calibrated, stable target. Per-spectrum counts vary widely within a "level".
...
skip-if-exists, `data/` is empty and rebuildable, and no data or checkpoints are
committed. Python 3.12, native Windows, with `sbi` (NPE+NSF), `jaxspec` (CPU JAX),
`ultranest`.
