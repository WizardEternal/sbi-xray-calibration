"""The coverage-vs-nominal panel: outputs/diagnostics/coverage_panel.png.

Reads the finished calibration artifacts in
``outputs/calibration/{faint,medium,bright}/`` and produces

    outputs/diagnostics/coverage_panel.png
...
Each panel's label names the recalibration shown and its mean-abs-deviation.
...
Regenerable, no config: ::
    ...
    .venv\\Scripts\\python.exe scripts\\make_coverage_panel.py

It reads outputs/calibration/<level>/{coverage_before_after,is_coverage}.npz and
summary.json and writes only outputs/diagnostics/coverage_panel.png. It
...
# load_level/choose_recal below look like scripts/make_summary_figure.py's
...
def main():
    ...
    for ax, d in zip(axes, data):
        ...
        # annotate the deviations + low-ESS in-panel
    ...
    out = _repo_root() / "outputs" / "diagnostics" / "coverage_panel.png"
