"""Reseed robustness pack: retrains the bright flow under three fresh seeds and
once with the epoch cap lifted, to test whether the bright-level NPE
over-confidence reproduces.

Per variant (seed 101 / 202 / 303 / uncapped) this script, all crash-resumable at
every stage:
  ...
  3. runs the reduced calibration suite      (SBC N=500 + coverage at the three
     nominal levels {50,68,90}); artifacts -> outputs/reseed/<variant>/;
  4. appends a one-line summary               (variant, cov@50/68/90, dev, SBC KS
     p min) to outputs/reseed/summary.jsonl.
...
Usage (repo venv, OMP_NUM_THREADS=4):
    .venv\\Scripts\\python.exe scripts\\run_reseed_pack.py --config configs\\reseed_101.yaml
    .venv\\Scripts\\python.exe scripts\\run_reseed_pack.py --config configs\\reseed_101.yaml --detect-spot
    .venv\\Scripts\\python.exe scripts\\run_reseed_pack.py --config configs\\reseed_uncapped.yaml

Writes only to data/sim/ (training datasets), outputs/models/<variant>_bright/
(checkpoints) and outputs/reseed/ (calibration artifacts + summary). It never
...
def _reseed_dir() -> Path:
    d = _repo_root() / "outputs" / "reseed"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _summary_path() -> Path:
    return _reseed_dir() / "summary.jsonl"
...
def ensure_dataset(cfg: dict) -> tuple[str, Path]:
    ...
    the production bright dataset (same draws as production, since the epoch-cap
    control changes only the epoch budget)."""
    variant = cfg["variant"]
...
# stage 3: reduced calibration suite (SBC N=500 + coverage @ {50,68,90})
...
def _cov_at(nominal, cov, target):
    ...
    scripts/run_calibration.py's ``_cov_at`` which returns it. Each keeps its own
...
def run_calibration(cfg: dict, ckpt_dir: Path, force: bool = False) -> dict:
    """Run the reduced calibration suite against the variant's checkpoint and
    write outputs/reseed/<variant>/calibration.json (skip-if-exists). Returns the
    calibration result dict."""
    variant = cfg["variant"]
    device = cfg.get("device", "cpu")
    out_dir = _reseed_dir() / variant
    ...
    # plain scalars so the summary line and the per-variant table read cleanly.
...
def append_summary_line(cal: dict):
    ...
    p min) to outputs/reseed/summary.jsonl. Skip-if-already-present."""
    variant = cal["variant"]
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Reseed robustness pack runner")
