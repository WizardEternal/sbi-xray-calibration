...
already run):
    .venv\\Scripts\\python.exe scripts\\make_nicer_fig.py
"""
import json
...
def main(argv=None):
    arf_path = REPO / "data" / "nicer" / "nicer.arf"
    if not arf_path.exists():
        print(f"[error] missing {arf_path}. Fetch the NICER response first:")
        print("  .venv\\Scripts\\python.exe scripts\\fetch_nicer_response.py")
        return 1
