"""The README summary figure: outputs/summary_figure.png.
...
Two panels, side by side, colorblind-safe (Okabe-Ito), dpi>=200:
  ...
  panel (a)  calibration: empirical coverage vs nominal credibility, raw NPE
             ...
             coverage panel collapsed onto one axes (one raw + one recal
...
outputs must be present locally:
    ...
    .venv\\Scripts\\python.exe scripts\\make_summary_figure.py
"""
...
# load_calibration/choose_recal below look like scripts/make_coverage_panel.py's
...
# Reseed robustness variants: the bright-level reseeds plus the uncapped
# retrain. Their raw coverage curves form the near-the-diagonal envelope showing
# that the production-flow over-confidence is a single-flow training artifact and
# not the count regime. Loaded read-only from outputs/reseed/<variant>/.
RESEED_VARIANTS = ["gonogo_seed101", "gonogo_seed202", "gonogo_seed303",
                   "gonogo_uncapped"]


def load_reseed_raw_band():
    ...
    for v in RESEED_VARIANTS:
        npz = root / "outputs" / "reseed" / v / "coverage_before_after.npz"
        if not npz.exists():
            continue
        ...
        if nominal_ref is None:
            nominal_ref = nominal
        curves.append(raw)
        # report the deviation with the per-param metric the reseed pack uses (mean
        # over params and levels of |per-param coverage - nominal|), not the
        # mean-curve deviation.
...
def plot_calibration(ax):
    ...
    # reseed/uncapped raw-coverage band (bright level): the evidence that the
    # over-confidence is a single-flow artifact. Drawn under the curves so it
    # reads as context.
    band = load_reseed_raw_band()
    if band is not None:
        nb, lo, hi, devs = band
...
def plot_detection(ax, level="medium"):
    ...
    # Per-spectrum detectors only (D1/D2) win the solid ROC curves, since those are the
    ...
    # (8 legend entries fit the panel at the existing fontsize.)
    for fam in order:
...
def main():
    # No suptitle: the interpretive caption lives in the README, not inside the
    # image (a baked-in text block distorts the tight bounding box and is
    # unreadable at thumbnail size anyway).
    fig, axes = plt.subplots(1, 2, figsize=(12.4, 5.6))
    cal_data = plot_calibration(axes[0])
    det_cells = plot_detection(axes[1], level="medium")
    fig.tight_layout()

    out = _repo_root() / "outputs" / "summary_figure.png"
