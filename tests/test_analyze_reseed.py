"""Unit tests for the reseed classification logic (scripts/analyze_reseed.py).

Run with the repo venv:
    .venv\\Scripts\\python.exe -m pytest -q tests/test_analyze_reseed.py

The classifier turns the three full-reseed bright variants' raw coverage
deviations, and the uncapped flow's, into the ROBUST / FRAGILE / MIXED call, and
interprets the epoch-cap control. These tests pin the rule against synthetic
summary rows so a refactor cannot silently flip it.
"""
...
def _load_verdict_module():
    """Import scripts/analyze_reseed.py as a module (scripts/ is not a package)."""
...
# reseed classification: ROBUST / FRAGILE / MIXED
...
def test_detect_spot_rows_ignored_by_classifier():
    V = _load_verdict_module()
    # spot-check rows (kind=='detect_spot') must not affect the classification
    rows = [
        _cal_row("gonogo_seed101", 0.113),
        _cal_row("gonogo_seed202", 0.098),
        _cal_row("gonogo_seed303", 0.121),
