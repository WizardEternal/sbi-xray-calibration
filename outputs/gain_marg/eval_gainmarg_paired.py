"""Paired gain-shift evaluation at medium counts, the statistically correct
successor to ``eval_gainmarg.py``.

An audit of ``eval_gainmarg.py`` found it:
  (A) unpaired: clean (seed 20300611) and shifted (seed 20300641) test
      populations were independent draws from the prior, so the observed
      Gamma-bias swings (0.005 fixed flow / 0.012 gain-marg flow) sit INSIDE
      the +/-0.023 noise floor of an unpaired comparison of two N=500 means
      (SE_diff = sqrt(SE_clean^2 + SE_shift^2) with SE ~ 0.016 each). The
      eval could not resolve an induced bias at all.
  (B) contaminated: raw ``post.sample(reject_outside_prior=False)`` +
      raw percentiles, with up to 55.8% of samples outside prior support on
      some spectra (concentrated on powerlaw_1_norm / kT), never filtered.

This script fixes both:

  1. PAIRED populations: ONE set of N_TEST theta draws (single seeded
     ``sample_prior`` call, mirroring what ``misspec._params_b4`` does
     internally; B4 adds no extra source parameters, so its "params" ARE
     the base nuisance draw). Each theta is folded at g=1.00 AND g=1.03
     through ``responses.gain_shift_obsconf`` (the exact misspec.py B4
     path), using ``simulate.fold_theta`` for the noiseless model counts.
     ``simulate_misspec_population`` itself re-draws theta from a fresh RNG
     every call, so it cannot be reused for a same-theta pair without
     duplicating its internals; we do that duplication explicitly here
     instead.
  2. Common random numbers: for each spectrum ``i`` the SAME per-spectrum
     seed (``POISSON_SEED_BASE + i``) reseeds ``np.random.default_rng``
     before drawing the Poisson realization at EACH of the two gain
     values, so the clean/shifted pair differs only by the response shift,
     not by independent shot noise on top of it.
  3. Sampling mirrors ``detect.py::posterior_predictive_replicates``
     (lines ~228-249) exactly: ``reject_outside_prior=True`` with a
     ``max_sampling_time`` cap; on exception or a short draw, fall back to
     unrejected sampling and top up; then CLIP to the training prior box
     before taking percentiles/medians.
  4. Param order for both checkpoints is asserted against PHYS_ORDER before
     any indexing.

Run (repo venv):
    .venv\\Scripts\\python.exe outputs\\gain_marg\\eval_gainmarg_paired.py
    .venv\\Scripts\\python.exe outputs\\gain_marg\\eval_gainmarg_paired.py --n-test 8 --max-sampling-time 3
        (quick wiring check; does not touch the committed N_TEST=500 config)

Never modifies eval_gainmarg.py or its outputs. Writes only:
    outputs/gain_marg/paired_gain_bias_medium.json
    outputs/gain_marg/paired_gain_bias_medium.png
"""
...
# ----------------------------------------------------------------------------
# Config, parameterized so a bright variant is a small edit later:
#   EXPOSURE_S -> 3534.0, FIXED_DIR/GM_DIR -> *_bright, SEED_OFFSET tag -> bright
# ----------------------------------------------------------------------------
FIXED_DIR = ROOT / "outputs" / "models" / "train_npe_prod_medium"
GM_DIR = ROOT / "outputs" / "gain_marg" / "model_medium"
...
# stable integer seeds (never builtin hash()); disjoint from eval_gainmarg.py's
# SEED+40000.., SEED+100/200 and from gen_and_train_gainmarg.py's SEED+2
SEED = 20260611
...
def run(n_test=N_TEST, n_samples=N_SAMPLES, max_sampling_time=MAX_SAMPLING_TIME,
    ...
    print(f"[gen done] {dt_gen:.1f}s median_counts clean={np.median(x_clean.sum(1)):.0f} "
    ...
    if write_outputs:
        ...
        fig.suptitle("Paired gain-shift eval (medium, ~986 counts): "
                    "same-theta CRN pairs, N=500, rejection+clip sampling", fontsize=12)
        fig.tight_layout(rect=[0, 0, 1, 0.97])
        figpath = OUT / "paired_gain_bias_medium.png"
        fig.savefig(figpath, dpi=130)
        plt.close(fig)
        print(f"[fig] {figpath}")

        jp = OUT / "paired_gain_bias_medium.json"
...
def main():
    global GAIN_CASES, OUT, SEED, SEED_THETA, SEED_POISSON_BASE, SEED_SAMPLE
