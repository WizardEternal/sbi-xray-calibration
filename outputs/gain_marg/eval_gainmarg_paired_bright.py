...
def run(n_test=N_TEST, n_samples=N_SAMPLES, max_sampling_time=MAX_SAMPLING_TIME,
    ...
    if write_outputs:
        ...
        if save_per_source:
            npz_path = OUT / "paired_gain_bias_bright_per_source.npz"
...
def _write_per_source_npz(npz_path, theta, flow_out, n_test):
    ...
    draw and the seeds, so two runs (e.g. capped vs uncapped fixed flow) can
    be confirmed to share the same underlying population before their FIXED
    arms are compared directly.
    """
    data = {
...
def main():
    ...
    ap.add_argument("--fixed-dir", default=None,
                    help="fixed-response (5-param) checkpoint dir; relative paths resolve "
                         "against the repo root. Default = outputs/models/train_npe_prod_bright, "
                         "the committed production flow. The gain-marginalized flow "
                         "(outputs/gain_marg/model_bright) is NOT affected by this.")
    ap.add_argument("--out", default=None,
                    help="directory for paired_gain_bias_bright.{json,png}; relative paths "
                         "resolve against the repo root. Default = outputs/gain_marg, the "
                         "committed production location. Filenames are unchanged.")
