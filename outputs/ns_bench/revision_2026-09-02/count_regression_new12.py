...
VERBATIM COPY of scripts/analyze_count_regression.py (committed, git-tracked),
with EXACTLY ONE CHANGE, commented below: the paired-pairs input path is taken
from sys.argv[1] instead of being hardcoded to
outputs/ns_bench/paired_gain_check.jsonl, so the identical statistic can be run
on the retired 12-pair file (gate) and on the new 12-pair
revision_2026-09-02/medium/pairs_merged.jsonl file without touching either
committed input. Column names (counts_clean, counts_gain, d_paired) are
identical between the two files, so no other change was needed.

Original docstring (unchanged below this point, describes the statistic):
---
...
Run (repo venv, from repo root):
    .venv\Scripts\python.exe outputs\ns_bench\revision_2026-09-02\count_regression_new12.py <path-to-pairs.jsonl>

If <path-to-pairs.jsonl> is omitted, defaults to the retired
outputs/ns_bench/paired_gain_check.jsonl (gate behaviour, identical to the
original script).
"""
...
ROOT = Path(__file__).resolve().parents[3]  # .../sbi-xray-calibration

# --- ONLY CHANGE vs scripts/analyze_count_regression.py: path from argv[1] ---
PAIRED = Path(sys.argv[1]) if len(sys.argv) > 1 else (
    ROOT / "outputs" / "ns_bench" / "paired_gain_check.jsonl"
)
# -----------------------------------------------------------------------------
...
def main():
    ...
    print(f"clean logZ-counts trend (from count_controlled.json, unchanged reference): "
